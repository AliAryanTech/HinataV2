...
let handler = async (m, {
    ...
    let two = ur[1]
    let three = ur[2]
    let listSections = themes.map((theme, index) => {
        let url = theme;
        let title = url.substring(url.lastIndexOf('/') + 1, url.lastIndexOf('.html'))
            .replace(/-/g, ' ')
    ...
    if (!one) {
        return conn.reply(m.chat, `⚠️ Masukkan nomor tema atau teks yang valid. Contoh penggunaan:\n\n*!photooxy [nomor tema]|[teks]*\n\n*Daftar Tema:*\n${listSections.join("\n")}`, m);
    }

    if (isNaN(one)) {
        return conn.reply(m.chat, `⚠️ Nomor tema tidak valid. Pilih nomor tema yang tersedia:\n\n*!photooxy [nomor tema]|[teks]*\n\n*Daftar Tema:*\n${listSections.join("\n")}`, m);
    }

    let index = parseInt(one);
